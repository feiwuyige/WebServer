...
void ApplyFriend::InitTipLbs()
{
    ...
    for(size_t i = 0; i < _tip_data.size(); i++){
        ...
        lb->SetState("normal", "hover", "pressed", "select_normal",
                     "select_hover", "select_pressed");
        lb->setObjectName("tip_lb");
...
void ApplyFriend::ShowMoreLabel(QString text)
{
...
{
    if(ui->lb_ed->text().trimmed().isEmpty()){
        return;
    }
