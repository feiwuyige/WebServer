...
void CSession::Send(char* msg, short max_length, short msgid)
{
	...
	//异步发送使用队列来保证有序，不会将数据发乱
	//所以如果目前队列中有元素，那就把自己放进队列即可，所以直接return
	//注意send_que_size是将发送节点压入队列之前队列的大小，只有为0的时候才会发送这个数据
...
void CSession::AsyncReadBody(int length)
{
	...
	asyncReadFull(length, [self, this, length](const boost::system::error_code& ec, std::size_t byteTransfered) {
		try {
			if (ec) {
...
{
	auto self = shared_from_this();
	//参数是const MutableBufferSequence& buffers,
	// BOOST_ASIO_COMPLETION_TOKEN_FOR(void (boost::system::error_code,std::size_t)) 回调函数
...
void CSession::HandleWrite(const boost::system::error_code& error, std::shared_ptr<CSession> shared_self)
{
	try {
		...
		std::cout << "Excepthio code: " << e.what() << std::endl;
	}
}
